...
-   定义：基于对象和事件驱动的客户端脚本语言
-   对象：类 -- 具有相同属性（有什么）和方法（能做什么）的集合对象   -- 类的实例化
-   面向过程=>面向对象
-   ECMAScript5->ECMAScript6（es5=>es6）
-   es5：一切皆对象
-   事件驱动：所有的页面效果都由事件触发
-   客户端脚本语言：浏览器解释型语言
...
- 规则 - 必须遵守的，不遵守就会报错
  ...
  - 由字母、数字、下划线、$符号组成，不能以数字开头（可以使用中文）
...
- parseInt()
  ...
  var isRight = parseInt(true);    //无法把布尔类型转换成数值类型，返回的结果是NaN
  ```

- parseFloat()

  ```
  parseFloat()把字符串转换成浮点数，无法转换布尔类型，返回的是NaN
  parseFloat()和parseInt非常相似，不同之处在与parseFloat会解析第一个. 遇到第二个.或者非数字结束。如果解析的内容里只有整数，则解析成整数
  ```
...
### 关系运算符(比较运算符)
    ...
    =赋值 ==只判断内容，===既判断内容,又判断类型
    !=只判断内容,不判断类型,!==全不等于,既判断内容,又判断类型
